...
function marcello_scavo_handle_contact_form() {
    ...
    if (empty($name) || empty($email) || empty($message)) {
        wp_send_json_error(['message' => __('Tutti i campi sono obbligatori', 'marcello-scavo')]);
    }
    
    if (!is_email($email)) {
        wp_send_json_error(['message' => __('Email non valida', 'marcello-scavo')]);
    }
    
    // Prepare email
    $admin_email = get_option('admin_email');
    $email_subject = sprintf(__('Nuovo messaggio da %s: %s', 'marcello-scavo'), $name, $subject);
    ...
    if (wp_mail($admin_email, $email_subject, $email_message, $headers)) {
        wp_send_json_success(['message' => __('Messaggio inviato con successo!', 'marcello-scavo')]);
    } else {
        wp_send_json_error(['message' => __('Errore nell\'invio del messaggio', 'marcello-scavo')]);
    }
}
...
function marcello_scavo_handle_newsletter() {
    ...
    if (wp_mail($admin_email, $subject, $message)) {
        wp_send_json_success(['message' => __('Iscrizione completata!', 'marcello-scavo')]);
    } else {
        wp_send_json_error(['message' => __('Errore nell\'iscrizione', 'marcello-scavo')]);
    }
}
...
function marcello_scavo_customize_register($wp_customize) {
    ...
    $wp_customize->add_section('hero_section', array(
        'title' => __('Sezione Hero', 'marcello-scavo'),
        'priority' => 30,
    ));
                ...
                <div style="display: flex; gap: 10px; align-items: flex-start;">
                    <div style="flex: 1;">
                        <input class="widefat placeholder-image-display" 
                               ...
                               type="text" 
                               value="<?php echo $image_id ? sprintf(__('Immagine ID: %d (%s)', 'marcello-scavo'), $image_id, basename($image_url)) : __('Nessuna immagine selezionata', 'marcello-scavo'); ?>" 
                               readonly
