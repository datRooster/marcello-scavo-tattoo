...
class Gallery3D {
    ...
    initCanvaStyle2D() {
        ...
        // Aggiungi scorciatoie da tastiera per il resize
    ...
    setup2DCardClicks() {
        document.addEventListener('click', (e) => {
            ...
            if (card && !actionBtn) {
                // Double click per resize mode
                if (this.lastClickTime && Date.now() - this.lastClickTime < 300) {
                    e.preventDefault();
                    this.toggleResizeMode(card);
                } else {
            ...
            // Click fuori per disattivare resize mode
            if (!card && !e.target.closest('.resize-handle, .resize-toolbar')) {
    ...
    handleRotation(e, card) {
        // Implementazione rotazione (opzionale)
        const rect = card.getBoundingClientRect();
    ...
    // Funzioni di utilità per il sistema di ridimensionamento
    getCardAspectRatio(card) {
    ...
    showResizePreview(card, width, height) {
        // Mostra anteprima durante il ridimensionamento
        let preview = card.querySelector('.resize-preview');
        if (!preview) {
    ...
    hideResizePreview(card) {
        const preview = card.querySelector('.resize-preview');
        if (preview) {
            preview.style.display = 'none';
        }
    }

    showSnapIndicators(x, y) {
        // Mostra indicatori di snap durante il ridimensionamento
        let indicators = document.querySelector('.snap-indicators');
        if (!indicators) {
        ...
        indicators.innerHTML = `
            <div class="snap-line vertical" style="left: ${x}px;"></div>
            <div class="snap-line horizontal" style="top: ${y}px;"></div>
        `;
        indicators.style.display = 'block';
    }

    hideSnapIndicators() {
        const indicators = document.querySelector('.snap-indicators');
        if (indicators) {
            indicators.style.display = 'none';
        }
    }

    getSnapPosition(value, snapSize = 20) {
        // Calcola posizione di snap più vicina
        return Math.round(value / snapSize) * snapSize;
    }

    isNearSnapPosition(value, snapSize = 20, threshold = 10) {
        // Verifica se un valore è vicino a una posizione di snap
        const snapPos = this.getSnapPosition(value, snapSize);
        return Math.abs(value - snapPos) <= threshold;
    }

    constrainToContainer(card, width, height) {
        // Limita le dimensioni al contenitore
        const container = card.closest('.artworks-grid');
        if (!container) return { width, height };
        
        const containerRect = container.getBoundingClientRect();
        const maxWidth = containerRect.width - 40; // Margine
        ...
        return { 
            width: Math.max(100, constrainedWidth), 
            height: Math.max(100, constrainedHeight) 
        };
    }

    showResizeTooltip(handle, text) {
        // Mostra tooltip sui punti di controllo
        let tooltip = handle.querySelector('.resize-tooltip');
        if (!tooltip) {
    ...
    addKeyboardShortcuts() {
        // Scorciatoie da tastiera per il resize
        document.addEventListener('keydown', (e) => {
            const activeCard = document.querySelector('.canva-card.resizable');
            ...
            switch (e.key) {
                ...
                case 'r':
                    if (e.ctrlKey || e.metaKey) {
                        e.preventDefault();
                        this.resetCardSize(activeCard);
                    }
                    break;
                case 'c':
                    if (e.ctrlKey || e.metaKey) {
                        e.preventDefault();
                        this.centerCard(activeCard);
                    }
                    break;
            }
        });
